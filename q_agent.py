...
class QAgent(Agent):
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if np.random.rand() < self.epsilon:
            ...
            # Wybór najlepszej akcji zgodnie z tablicą Q
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        best_future_q = np.max(self.q_table[new_state]) if not done else 0
        self.q_table[state, action] += self.lr * (reward + self.gamma * best_future_q - self.q_table[state, action])
